...
class Emergency:
	def __init__(self, parent):
		...
		self.gesturechart = cv2.imread("images\\e1.jpg")
...
class Shortcut:
	def __init__(self, parent):
		...
		self.label.place(x = 100 , y = 10)
		self.gesturechart = cv2.imread("images\\c1.jpg")
		self.panel3 = Label(self.root1)
		self.panel3.place(x =50 , y = 45 , width = 400 , height = 600)
		self.im1 = cv2.cvtColor(self.gesturechart, cv2.COLOR_BGR2RGBA)
		self.im1 = cv2.resize(self.im1, (400, 500))
...
class Sign_to_text:
	...
	def __init__(self, obj):
		...
		#enter buuton
		self.bt0 = Button(self.root ,text = "Emergency" ,font = ("Courier",15,"bold"),command = self.Emergency , bg = "gray9", fg = "deepskyblue2")
		self.bt0.place(x=530, y=500)

		self.bt01 = Button(self.root ,text = "Shortcuts" ,font = ("Courier",15,"bold"),command = self.Shortcut , bg = "gray9", fg = "deepskyblue2")
	...
	def predict(self,test_image):
		...
		if(self.ct[self.current_symbol] > 45):
			...
			if self.Emergency_flag:
				with open("dictionary/emergency_phrases.csv") as f_obj:
					emreader = csv.reader(f_obj, delimiter=',')
					for line in emreader:
						if self.current_symbol in line[0]:
							self.sentence = self.sentence + " " + str(line[1])  
				

			if self.shortcut_flag:
				with open("dictionary/Conversation_phrases.csv") as f_obj:
					emreader = csv.reader(f_obj, delimiter=',')
					for line in emreader:
						if self.current_symbol in line[0]:
							self.sentence = self.sentence + " " + str(line[1])  

			if self.Emergency_flag == False and self.shortcut_flag == False:
	...
	def Emergency_suggestions(self):
		self.bt1.config(text = "Call" ,font = ("Courier",15))
		self.bt2.config(text = "You",font = ("Courier",15))
		self.bt3.config(text = "Me",font = ("Courier",15))
		self.bt4.config(text = "Police",font = ("Courier",15))
		self.bt5.config(text = "Safe",font = ("Courier",15))
		self.bt6.config(text = "Help",font = ("Courier",15))
		self.bt7.config(text = "Evacuate",font = ("Courier",15))
		self.bt8.config(text = "Ambulance",font = ("Courier",15))
		self.bt9.config(text = "Hospital",font = ("Courier",15))
		self.bt10.config(text = "Firefighter",font = ("Courier",15))
		self.bt11.config(text = "Bleeding",font = ("Courier",15))
		self.bt12.config(text = "House" ,font = ("Courier",15))
	...
	def action1(self):
		if self.Emergency_flag:
			self.sentence =self.sentence + " " +  self.bt1['text']
		else:
			dic = self.list2
			if (len(dic)) > 0:
				self.word = ""
	...
	def action2(self):
		if self.Emergency_flag:
			self.sentence =self.sentence + " " +  self.bt2['text']
		else:
			dic = self.list2
			if (len(dic)) > 1:
	...
	def action3(self):
		if self.Emergency_flag:
			self.sentence =self.sentence + " " +  self.bt3['text']
		else:
			dic = self.list2
			if (len(dic)) > 2:
	...
	def action4(self):
		if self.Emergency_flag:
			self.sentence =self.sentence + " " +  self.bt4['text']
		else:
			dic = self.list2
			if (len(dic)) > 3:
	...
	def action5(self):
		if self.Emergency_flag:
			self.sentence =self.sentence + " " +  self.bt5['text']
		else:
			dic = self.list2
			if (len(dic)) > 4:
	...
	def action6(self):
		if self.Emergency_flag:
			self.sentence =self.sentence + " " +  self.bt6['text']
		else:
			dic = self.list2
			if (len(dic)) > 5:
	...
	def action7(self):
		if self.Emergency_flag:
			self.sentence =self.sentence + " " +  self.bt7['text']
		else:
			dic = self.list2
			if (len(dic)) > 6:
	...
	def action8(self):
		if self.Emergency_flag:
			self.sentence =self.sentence + " " +  self.bt8['text']
		else:
			dic = self.list2
			if (len(dic)) > 7:
	...
	def action9(self):
		if self.Emergency_flag:
			self.sentence =self.sentence + " " +  self.bt9['text']
		else:
			dic = self.list2
			if (len(dic)) > 8:
	...
	def action10(self):
		if self.Emergency_flag:
			self.sentence =self.sentence + " " +  self.bt10['text']
		else:
			dic = self.list2
			if (len(dic)) > 9 :
	...
	def action11(self):
		if self.Emergency_flag:
			self.sentence =self.sentence + " " +  self.bt11['text']
		else:
			dic = self.list2
			if (len(dic)) > 10:
	...
	def action12(self):
		if self.Emergency_flag:
			self.sentence =self.sentence + " " +  self.bt12['text']
		else:
			dic = self.list2
			if (len(dic)) > 11:
